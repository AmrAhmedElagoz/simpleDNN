...
static std::string cuda_return_error_string(cudaError_t error)
{
    switch (error)
    {
         ...
         * ::cudaErrorInvalidConfiguration, this error usually indicates that the  return "";
...
void cuda_call_check(cudaError_t result, const char *function_name, const char *filename, int line_number)
{
    if (result)
    {
        std::cout << "\033[1;31m[CUDA Error]:\033[0m " << cuda_return_error_string(result) << " (" <<
        result << "), " << "Filename: " << filename << ", " << "Line Number" << line_number << ", " << 
